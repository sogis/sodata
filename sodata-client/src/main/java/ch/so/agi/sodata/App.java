...
import org.dominokit.domino.ui.tabs.Tab.ActivationHandler;
...
//        }
        ...
        MaplayerElement mlElement = new MaplayerElement(DATA_BASE_URL);
        mapsTab.appendChild(mlElement.element());
        DataElement dElement = new DataElement(DATA_BASE_URL);
        dataTab.appendChild(dElement.element());
        ModelElement mElement = new ModelElement(DATA_BASE_URL);
        modelsTab.appendChild(mElement.element());
        
        topLevelContent.appendChild(tabsPanel.element());
        
        tabsPanel.addEventListener("searchStringChanged", new EventListener() {
            @Override
            public void handleEvent(Event evt) {
                searchTerms = ((CustomEvent) evt).detail.toString();
            }
        });
        
        tabsPanel.addActivationHandler(new ActivationHandler() {
            @Override
            public void onActiveStateChanged(Tab tab, boolean active) {
                if (searchTerms != null) {
                    if (mapsTab.isActive()) {
                        mlElement.setSearchText(searchTerms);
                    } else if (dataTab.isActive()) {
                        dElement.setSearchText(searchTerms);
                    } else if (modelsTab.isActive()) {
                        mElement.setSearchText(searchTerms);
                    }
                }
            }            
        });
...
//            @Override
//            public void handleEvent(Event evt) {
//                console.log("scheiss click");
//                if (searchTerms != null && searchTerms.trim().length() > 0) {
//                    if (mapsTab.isActive() && mapsTab != tabsPanel.getActiveTab()) {
//                        mlElement.setSearchText(searchTerms);
//                    } else if (dataTab.isActive() && dataTab != tabsPanel.getActiveTab()) {
//                        console.log(searchTerms);
//                        dElement.setSearchText(searchTerms);
//                    } else if (modelsTab.isActive() && modelsTab != tabsPanel.getActiveTab()) {
//                        mElement.setSearchText(searchTerms);
//                    }
//                }
//            }
