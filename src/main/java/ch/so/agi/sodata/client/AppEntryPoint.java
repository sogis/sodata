...
//        HTMLDivElement adiv = div().id("map").element();

//        MapCard mapCard = new MapCard("map", "atitle");
        //body().add(mapCard.element());
//        body().add(adiv);
        
//        Map map = MapPresets.getBlackAndWhiteMap("map");
        
      
        
        Button btn = Button.createPrimary("PRIMARY").linkify().style().setMargin(px.of(5)).setMaxWidth(px.of(120)).get();
        body().add(btn.element());
...
//
//        HTMLDivElement adiv = div().id("map").element();
//        modal.getBodyElement().appendChild(adiv);
//
...
//        Map map = MapPresets.getBlackAndWhiteMap(adiv.id);
